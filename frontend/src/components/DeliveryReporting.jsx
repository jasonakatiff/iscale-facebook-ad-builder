...
export function DeliveryReporting() {
    ...
    return <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-surface border border-line rounded-xl p-5 space-y-4">
            ...
            <p className="text-sm text-secondary">7-day click / 1-day view attribution, by conversion date. Dates follow each account’s timezone; spend stays in the account’s currency. Previously collected history beyond 28 days is retained.</p>
            ...
            {report?.data.length > 0 && <div className="overflow-x-auto"><table className="w-full text-sm text-left"><thead><tr className="text-muted border-b"><th className="py-3">Ad / account</th><th>Date</th><th>Delivery</th><th className="text-right">Impressions</th><th className="text-right">Clicks</th><th className="text-right">Spend</th><th className="pl-4">Last imported</th></tr></thead><tbody>{report.data.map(row => <tr key={row.id} className="border-b align-top"><td className="py-3 pr-4"><p className="font-medium">{row.name}</p><p className="text-xs text-muted">{row.account_id} · Ad {row.fb_ad_id}</p></td><td className="py-3 pr-4 whitespace-nowrap">{row.report_date}<p className="text-xs text-muted">{row.account_timezone}</p></td><td className="py-3 pr-4">{row.effective_status || 'Awaiting status'}</td><td className="py-3 text-right">{row.impressions}</td><td className="py-3 text-right">{row.clicks}</td><td className="py-3 text-right whitespace-nowrap">{row.currency} {row.spend.replace(/(\.\d*?[1-9])0+$|\.0+$/, '$1')}</td><td className="py-3 pl-4 whitespace-nowrap">{formatDeliveryTime(row.imported_at)}</td></tr>)}</tbody></table></div>}
