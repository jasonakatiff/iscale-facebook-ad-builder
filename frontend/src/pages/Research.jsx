                ...
                <div className="space-y-6">
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="text-secondary mt-1">Select a vertical to view and manage searches</p>
                        </div>
                        <button
                            onClick={() => setShowVerticalModal(true)}
                            className="px-4 py-2 bg-brand text-white rounded-lg hover:bg-brand-hover shadow-sm"
                        >
                            + New Vertical
                        </button>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {verticals.map((vertical) => (
                                <button
                                    ...
                                    className="bg-panel p-6 rounded-lg border-2 border-line shadow-sm hover:border-indigo-500 hover:shadow-md transition-all text-left"
                                >
                                    <h3 className="font-semibold text-xl text-foreground mb-2">{vertical.name}</h3>
                                    {vertical.description && (
                                        <p className="text-secondary text-sm mb-4">{vertical.description}</p>
                                    )}
                                    <div className="flex items-center justify-between text-sm">
                                        <span className="text-muted">
                                            Created {new Date(vertical.created_at).toLocaleDateString()}
                                        </span>
                                        <span className="text-highlight font-medium">View →</span>
                                    </div>
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            ...
                            {selectedVertical.description && (
                                <p className="text-secondary">{selectedVertical.description}</p>
                            )}
                        </div>
                    </div>
                            ...
                            <button
                                ...
                                className={`px-4 py-2 font-medium border-b-2 transition-colors ${
                                    verticalTab === 'aggregated'
                                        ? 'border-indigo-600 text-highlight'
                            ...
                            <div className="flex justify-between items-center mb-4">
                                ...
                                {aggregatedAds.length > 0 && (
                                    <input
                                        ...
                                        className="px-4 py-2 border border-line-strong rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none w-64"
                                    />
                                )}
                            </div>
                            {aggregatedAds.length === 0 ? (
                                <p className="text-muted text-center py-8">No ads yet. Run a search to see aggregated results.</p>
                            ) : (
                                <div className="space-y-2">
                                    {(() => {
                                        ...
                                        return filteredPages.length === 0 ? (
                                            <p className="text-muted text-center py-8">No pages match your filter.</p>
                                        ) : filteredPages.map((page) => (
                                        <div key={page.page_id} className="border border-line rounded-lg overflow-hidden">
                                            <div className="px-4 py-3 bg-subtle flex items-center justify-between">
                                                <div className="flex-1 flex items-center gap-3">
                                                    <span
                                                        onClick={() => togglePageExpansion(page.page_id)}
                                                        className="text-secondary hover:text-foreground transition-transform cursor-pointer select-none"
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-4 mt-1 text-sm">
                                                            ...
                                                            <div className="flex items-center gap-3 text-xs">
                                                                {page.image_count > 0 && (
                                                                    <span className="inline-flex items-center px-2 py-1 bg-info-soft text-info rounded">
                                                                        📷 {page.image_count} images
                                                                    </span>
                                                                )}
                                                                {page.video_count > 0 && (
                                                                    <span className="inline-flex items-center px-2 py-1 bg-highlight-soft text-highlight rounded">
                                                                        🎥 {page.video_count} videos
                                                                    </span>
                                                                )}
                                                                {page.carousel_count > 0 && (
                                                                    <span className="inline-flex items-center px-2 py-1 bg-success-soft text-success rounded">
                                                ...
                                                <button
                                                    ...
                                                    className="px-3 py-1 text-sm bg-danger-soft text-danger rounded hover:bg-danger-soft"
                                                >
                                                    Block Page
                                                </button>
                                            </div>
                                            ...
                                            {expandedPages.has(page.page_id) && (
                                                <div className="px-4 py-3 bg-panel border-t border-line">
                                                    {!pageAds[page.page_id] ? (
                                                        <div className="text-center py-4 text-muted">Loading ads...</div>
                                                    ) : pageAds[page.page_id].length === 0 ? (
                                                        <div className="text-center py-4 text-muted">No ads found</div>
                                                    ) : (
                                                        <div className="space-y-3">
                                                            {(() => {
                                                                ...
                                                                return filteredAds.length === 0 ? (
                                                                    <div className="text-center py-4 text-muted">No ads match your filter.</div>
                                                                ) : filteredAds.map((ad) => (
                                                                <div key={ad.id} className="border border-line rounded p-3 hover:shadow-sm">
                                                                    <div className="flex justify-between items-start mb-2">
                                                                        <div className="flex-1">
                                                                            ...
                                                                            {ad.ad_copy && (
                                                                                <p className="text-sm text-secondary mt-1">{ad.ad_copy}</p>
                                                                            )}
                                                                        </div>
                                                                        {ad.media_type && (
                                                                            <span className="ml-2 px-2 py-1 text-xs bg-inset text-secondary rounded">
                                                                                {ad.media_type}
                                                                            </span>
                                                                        )}
                                                                    </div>
                                                                    <div className="flex items-center justify-between text-xs text-muted mt-2">
                                                                        <div className="flex items-center gap-3">
                                                                            {ad.cta_text && (
                                                                                <span className="px-2 py-1 bg-info-soft text-info rounded">
                                                                                    CTA: {ad.cta_text}
                                                                                </span>
                                                                            )}
                        ...
                        <form onSubmit={handleScrape} className="space-y-4">
                            <div className="flex flex-col sm:flex-row gap-4">
                                <input
                                    ...
                                    className="flex-1 p-3 border border-line-strong rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                                />
                                <select
                                    value={country}
                                    onChange={(e) => setCountry(e.target.value)}
                                    className="p-3 border border-line-strong rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none bg-panel"
                                >
                                    {COUNTRIES.map((c) => (
                            ...
                            <div>
                                <label className="block text-sm font-medium text-secondary mb-2">Search Type</label>
                                <select
                                    value={searchType}
                                    onChange={(e) => setSearchType(e.target.value)}
                                    className="w-full p-3 border border-line-strong rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none bg-panel"
                                >
                            ...
                            <div className="bg-info-soft border border-info-line rounded-lg p-3 text-sm space-y-2">
                                ...
                                {rateLimit && (
                                    <div className="flex items-center justify-between pt-2 border-t border-info-line">
                                        <span className="text-secondary">
                                            Rate Limit: <strong className={rateLimit.remaining < 50 ? 'text-danger' : 'text-success'}>
                            ...
                            {loading && progressMessage && (
                                <div className="bg-warning-soft border border-warning-line rounded-lg p-3 text-sm">
                                    <div className="flex items-center gap-2">
                                        <svg className="animate-spin h-5 w-5 text-warning" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                        </svg>
                                        <span className="text-secondary">{progressMessage}</span>
                                    </div>
                                </div>
                            )}
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full px-6 py-3 bg-brand text-white font-semibold rounded-lg hover:bg-brand-hover transition-colors disabled:opacity-50"
                            >
                            ...
                            <div className="bg-panel rounded-lg border border-line p-6">
                                ...
                                {savedSearches.filter(s => s.vertical_id === selectedVertical.id).length === 0 ? (
                                    <p className="text-muted text-center py-8">No searches yet for this vertical.</p>
                                ) : (
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {search.search_type !== 'one_time' && (
                                                                <span className={`px-2 py-1 text-xs rounded ${
                                                                    search.search_type === 'scheduled_daily' ? 'bg-info-soft text-info' :
                                                                    search.search_type === 'scheduled_weekly' ? 'bg-highlight-soft text-highlight' :
                                                                    'bg-inset text-secondary'
                                                                }`}>
                    ...
                    <div className="flex justify-between items-center">
                        <div>
                            <button
                                ...
                                className="text-highlight hover:text-highlight mb-2 flex items-center gap-1"
                            >
                                ← Back to Searches
                            </button>
                    ...
                    {(() => {
                        ...
                        return pageGroups.length === 0 ? (
                            <div className="text-center py-16 bg-panel rounded-lg border border-line">
                                <p className="text-muted">No ads found in this vertical</p>
                            </div>
                        ) : (
                            <div className="space-y-2">
                                {pageGroups.map(({ pageName, ads, totalAds }) => (
                                    <div key={pageName} className="bg-panel rounded-lg border border-line overflow-hidden">
                                        <button
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="text-left">
                                                    ...
                                                    <p className="text-sm text-muted">{totalAds} unique ads</p>
                                                </div>
                                            </div>
                                            <button
                                                onClick={(e) => {
                                            ...
                                            <div className="border-t border-line">
                                                <table className="min-w-full">
                                                    <thead className="bg-subtle">
                                                        <tr>
                                                            <th className="px-4 py-2 text-left text-xs font-medium text-muted uppercase">Headline</th>
                                                            <th className="px-4 py-2 text-left text-xs font-medium text-muted uppercase">Ad Copy</th>
                                                            <th className="px-4 py-2 text-left text-xs font-medium text-muted uppercase">CTA</th>
                                                            <th className="px-4 py-2 text-left text-xs font-medium text-muted uppercase">Searches</th>
                                                            <th className="px-4 py-2 text-left text-xs font-medium text-muted uppercase">Seen</th>
                                                            <th className="px-4 py-2 text-left text-xs font-medium text-muted uppercase">Started</th>
                                                            <th className="px-4 py-2 text-left text-xs font-medium text-muted uppercase">Actions</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody className="divide-y divide-line">
                                                        {ads.map((ad) => (
                                                            <tr key={ad.id} className="hover:bg-subtle">
                                                                <td className="px-4 py-2 text-sm text-secondary max-w-xs">
                                                                    <div className="line-clamp-2">{ad.headline || '-'}</div>
                                                                </td>
                                                                <td className="px-4 py-2 text-sm text-secondary max-w-md">
                                                                    <div className="line-clamp-2">{ad.ad_copy || '-'}</div>
                                                                </td>
                                                                <td className="px-4 py-2 text-sm">
                                                                    {ad.cta_text && (
                                                                        <span className="inline-block px-2 py-1 bg-highlight-soft text-highlight rounded text-xs">
                                                                            {ad.cta_text}
                                                                        </span>
                                                                    )}
                                                                </td>
                                                                <td className="px-4 py-2 text-sm">
                                                                    <div className="flex gap-1 flex-wrap">
                                                                        {Array.from(ad.searches).map((search, idx) => (
                                                                            <span key={idx} className="px-2 py-1 bg-info-soft text-info rounded text-xs">
                                                                                {search}
                                                                            </span>
                                                                        ))}
                                                                    </div>
                                                                ...
                                                                <td className="px-4 py-2 text-sm text-secondary text-center">
                                                                    <span className="inline-flex items-center px-2 py-1 bg-success-soft text-success rounded text-xs font-semibold">
                                                                        {ad.seen_count || 1}x
                                                                    </span>
                                                                </td>
                    ...
                    <div className="bg-panel rounded-lg p-6 max-w-md w-full mx-4">
                        ...
                        <div className="space-y-4">
                            <input
                                ...
                                className="w-full p-3 border border-line-strong rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                                autoFocus
                            />
                            <textarea
                        ...
                        <div className="flex gap-2 mt-6">
                            <button
                                ...
                                className="flex-1 px-4 py-2 border border-line-strong text-secondary rounded hover:bg-subtle"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={handleCreateVertical}
                                className="flex-1 px-4 py-2 bg-brand text-white rounded hover:bg-brand-hover"
                            >
                                Create
                            </button>
                        </div>
