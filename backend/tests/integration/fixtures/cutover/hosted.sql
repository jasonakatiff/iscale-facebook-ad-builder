...
CREATE TABLE delivery_settings (
	...
	max_post_retries INTEGER DEFAULT '3' NOT NULL,
	max_read_retries INTEGER DEFAULT '3' NOT NULL,
	status_interval_seconds INTEGER DEFAULT '300' NOT NULL,
	performance_interval_seconds INTEGER DEFAULT '14400' NOT NULL,
	stable_status_interval_seconds INTEGER DEFAULT '3600' NOT NULL,
	lookback_days INTEGER DEFAULT '2' NOT NULL,
	reconcile_days INTEGER DEFAULT '35' NOT NULL,
	reconcile_interval_hours INTEGER DEFAULT '24' NOT NULL,
	metadata_cache_hours INTEGER DEFAULT '24' NOT NULL,
	api_requests_per_minute INTEGER DEFAULT '120' NOT NULL,
	import_requests_per_minute INTEGER DEFAULT '60' NOT NULL,
	account_requests_per_minute INTEGER DEFAULT '60' NOT NULL,
	api_daily_request_limit INTEGER DEFAULT '10000' NOT NULL,
	api_max_concurrency INTEGER DEFAULT '2' NOT NULL,
	api_usage_pause_percent INTEGER DEFAULT '80' NOT NULL,
	async_poll_seconds INTEGER DEFAULT '30' NOT NULL,
	...
	CONSTRAINT ck_delivery_post_retries CHECK (max_post_retries BETWEEN 0 AND 10),
	...
	CONSTRAINT ck_delivery_sync_controls CHECK (lookback_days BETWEEN 1 AND 90 AND reconcile_days BETWEEN lookback_days AND 90 AND reconcile_interval_hours BETWEEN 1 AND 168 AND metadata_cache_hours BETWEEN 1 AND 168 AND stable_status_interval_seconds BETWEEN status_interval_seconds AND 86400 AND async_poll_seconds BETWEEN 15 AND 300),
	CONSTRAINT ck_delivery_request_controls CHECK (api_requests_per_minute BETWEEN 1 AND 10000 AND import_requests_per_minute BETWEEN 1 AND api_requests_per_minute AND account_requests_per_minute BETWEEN 1 AND 10000 AND api_daily_request_limit BETWEEN 1 AND 1000000 AND api_max_concurrency BETWEEN 1 AND 10 AND api_usage_pause_percent BETWEEN 10 AND 95)
);

CREATE TABLE delivery_api_requests (
	platform VARCHAR(12) DEFAULT 'meta' NOT NULL,
...
CREATE INDEX ix_delivery_api_requests_platform ON delivery_api_requests (platform);
...
CREATE INDEX ix_creative_events_asset_id ON creative_events (asset_id);
...
CREATE TABLE delivery_post_attempts (
	id VARCHAR(36) NOT NULL,
	job_id VARCHAR(36) NOT NULL,
	started_at TIMESTAMP WITH TIME ZONE NOT NULL,
	finished_at TIMESTAMP WITH TIME ZONE,
...
CREATE INDEX ix_delivery_post_attempts_job_id ON delivery_post_attempts (job_id);

CREATE INDEX ix_delivery_post_attempts_started_at ON delivery_post_attempts (started_at);
...
CREATE TABLE delivery_notifications (
	...
	PRIMARY KEY (id),
	CONSTRAINT uq_delivery_failure_notification UNIQUE (user_id, failure_id),
	FOREIGN KEY(job_id) REFERENCES delivery_jobs (id) ON DELETE CASCADE,
	FOREIGN KEY(user_id) REFERENCES users (id) ON DELETE CASCADE
);

CREATE INDEX ix_delivery_notifications_user ON delivery_notifications (user_id, read_at, created_at);
