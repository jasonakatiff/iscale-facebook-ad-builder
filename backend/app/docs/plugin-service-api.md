...
Download `/api/v1/plugins/example-worker` after signing in. The included worker requires `BREADWINNER_API_ORIGIN` and `BREADWINNER_PLUGIN_WORKER_KEY` in its process environment without writing them to disk. Set the origin to the Backend public domain of the installation that issued the worker key, including `https://` and without `/api/v1`. There is no default destination: the worker exits before making requests when the origin is missing. Run `python3 plugin-worker.py --once` to process one queued Service Starter job; omit `--once` to poll until interrupted. Plain HTTP is accepted only for an explicitly configured loopback origin during development.
