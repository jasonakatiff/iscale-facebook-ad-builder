...
AI generation requires configured provider keys. Personal Meta OAuth requires deployment-level Meta app setup; a managed Meta connection can still be available. The workflow prototype uses sample data and is distinct from live workflows. Campaign Reporting contains imported Meta daily snapshots and separate LeadRouter lifetime counts; the [Posting queue guide](/api/v1/help/docs/posting-queue) explains coverage and refresh intervals. Some General Settings controls remain demonstrations.
